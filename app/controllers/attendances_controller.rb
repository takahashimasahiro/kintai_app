  ...
  def update
    ...
    @change_rows.map do |i|
      ...
      @attend = AttendanceTime.find_or_initialize_by(
        user_id: @current_user.owner? ? params[:select_user] : @current_user.id,
      ...
      # 有給休暇申請を行う
      if AttendanceTime::vacation?(params[:"status_#{i}"])
      ...
      if AttendanceTime::vacation?(@attend.status) &&
          !AttendanceTime::vacation?(params[:"status_#{i}"])
